...
(function () {
  ...
  // ── ACTION EXECUTOR ────────────────────────────────────────────
  window.__omni_act__ = function (action, ref, value, snapshotIndex, labelHint = '') {
    function findEl(r) {
      const attempts = [];
      const item = (snapshotIndex || []).find(i => i.ref === r);
      if (!item) {
        attempts.push('ref_missing_in_snapshot');
        if (labelHint) {
          attempts.push('label_hint_exact');
          const hinted = findBySemanticLabel('button', labelHint) || findBySemanticLabel('input', labelHint);
          if (hinted) return { el: hinted, reason: null, attempts };
        }
        return { el: null, reason: 'REF_NOT_IN_SNAPSHOT', attempts };
      }
      ...
      if (labelHint) {
        attempts.push('label_hint_fallback');
        const hinted = findBySemanticLabel(item.role, labelHint) || findBySemanticLabel('button', labelHint) || findBySemanticLabel('input', labelHint);
        if (hinted) return { el: hinted, reason: null, attempts };
      }
      if (item.role === 'button' || /search/i.test(item.label || labelHint || '')) {
        attempts.push('search_button_heuristic');
        const searchBtn = Array.from(document.querySelectorAll('button,input[type="submit"],input[type="button"],[role="button"]'))
          .find(b => {
            const t = normalizeMatchLabel(
              b.getAttribute('aria-label') || b.getAttribute('value') || b.getAttribute('title') || b.innerText || ''
            );
            return /\bsearch\b/.test(t);
          });
        if (searchBtn) return { el: searchBtn, reason: null, attempts };
      }
